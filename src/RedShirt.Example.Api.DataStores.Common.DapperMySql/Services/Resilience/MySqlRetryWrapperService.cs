...
using RedShirt.Example.Api.DataStores.Common.Exceptions;

namespace RedShirt.Example.Api.DataStores.Common.DapperMySql.Services.Resilience;

/// <summary>
///     Retries MySQL / Dapper operations that fail with expected transient exceptions,
///     then surfaces remaining failures as <see cref="ApiDatabaseException" />.
/// </summary>
public interface IMySqlRetryWrapperService
{
    /// <summary>
    ///     Executes <paramref name="func" /> with retry for expected transient MySQL failures.
    /// </summary>
    ...
    /// </exception>
    /// <exception cref="ApiDatabaseException">
    ///     Thrown when <paramref name="func" /> ultimately fails. <see cref="ApiDatabaseException.CouldBeTransient" />
...
/// <summary>
///     Polly v8-based retry wrapper for MySQL / Dapper calls.
///     Retries when <see cref="IMySqlExceptionArbiterService" /> reports an expected transient failure,
///     using exponential backoff via <see cref="ISleepService" />.
///     Intended eventual replacement for <c>PolicyHelper.GetRetryPolicy</c>.
/// </summary>
/// <param name="exceptionArbiterService">Classifies MySQL-related exceptions as expected/transient.</param>
...
internal sealed class MySqlRetryWrapperService(
    IMySqlExceptionArbiterService exceptionArbiterService,
    ILogger<MySqlRetryWrapperService> logger,
    ISleepService sleepService)
    : IMySqlRetryWrapperService
{
    private const int MySqlRetryCount = 3;
    ...
    private ResiliencePipeline GetRetryPipeline()
    {
        return _retryPipeline ??= new ResiliencePipelineBuilder()
            ...
            {
                MaxRetryAttempts = MySqlRetryCount,
                ShouldHandle = args =>
                {
                ...
                OnRetry = async args =>
                {
                    logger.LogWarning(args.Outcome.Exception,
                        "Retrying MySQL operation after attempt {AttemptNumber}",
                        args.AttemptNumber);
    ...
    ///     If wrapping was appropriate, then will be <see cref="ApiDatabaseException" /> wrapped around the
    ...
    private bool TryGetWrappedException(Exception exception, out Exception? wrappedException)
    {
        ...
        if (report.AlreadyHandled && exception is ApiDatabaseException)
        {
            return false;
        }
        ...
        wrappedException = new ApiDatabaseException(exception)
        {
