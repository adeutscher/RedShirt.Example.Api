...
using RedShirt.Example.Api.Common.Distributed.Configuration;
...
namespace RedShirt.Example.Api.Common.Distributed.Services.Redis;
    ...
    public async Task<IAbstractedLock> GetLockAsync(string lockName, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            return new DistributedLock(retryWrapper,
                await redisLock.TryAcquireAsync(cancellationToken: cancellationToken));
        }
