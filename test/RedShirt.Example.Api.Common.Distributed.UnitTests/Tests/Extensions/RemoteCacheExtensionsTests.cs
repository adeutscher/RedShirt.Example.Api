...
public class RemoteCacheExtensionsTests
{
    ...
    public async Task SetObjectAsync_SerializesAndStoresJson()
    {
        ...
        remoteCache
            .Setup(c => c.SetStringAsync(key, It.IsAny<string?>(), expiry, TestContext.Current.CancellationToken))
        ...
        remoteCache.Verify(
            c => c.SetStringAsync(key, It.IsAny<string?>(), expiry, TestContext.Current.CancellationToken),
            Times.Once);
        remoteCache.VerifyNoOtherCalls();
    }
