...
using RedShirt.Example.Api.DataStores.Common.DapperMySql.Models;
...
namespace RedShirt.Example.Api.DataStores.Common.DapperMySql.UnitTests.Tests.Services.Resilience;

public class MySqlRetryWrapperServiceTests
{
    private static MySqlExceptionArbiterReport TransientReport()
    {
    ...
    private static MySqlExceptionArbiterReport NonTransientReport()
    {
        return new MySqlExceptionArbiterReport
        {
            AlreadyHandled = false,
    ...
    private static MySqlExceptionArbiterReport UnexpectedReport(bool couldBeTransient = false)
    {
        return new MySqlExceptionArbiterReport
        {
            AlreadyHandled = false,
    ...
    {
        var inner = new TaskCanceledException("connector-style timeout");
        var arbiter = new Mock<IMySqlExceptionArbiterService>(MockBehavior.Strict);
        ...
        var thrown = await Assert.ThrowsAsync<ApiDatabaseException>(() => wrapper.RunAsync<string>(
            _ => throw inner,
            TestContext.Current.CancellationToken));
