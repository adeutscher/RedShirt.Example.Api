...
#
# Usage:
#   ./bar-rotate-oauth-credentials.sh [new-secret] [new-access-token]
#
# Examples:
#   ./bar-rotate-oauth-credentials.sh
#   ./bar-rotate-oauth-credentials.sh my-new-secret my-new-token
#
# Pair with ./bar-set-ssm-oauth-secret.sh (SSM only → token 401) then this script (SSM + WireMock → recovery).
...
while IFS= read -r stub; do
    ...
    # Token success stub: replace client_secret=… bodyPatterns and access_token in response.
    if jq -e '
    ...
    # API stubs that require Authorization: Bearer …
