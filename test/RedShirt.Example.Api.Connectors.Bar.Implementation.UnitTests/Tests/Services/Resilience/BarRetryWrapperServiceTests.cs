...
public class BarRetryWrapperServiceTests
{
    private static BarRetryWrapperService CreateSut(
        IBarExceptionArbiterService? arbiter = null,
        ...
        int? retryCount = 3)
    {
        ...
        return new BarRetryWrapperService(
            arbiter ?? new BarExceptionArbiterService(),
            NullLogger<BarRetryWrapperService>.Instance,
            sleep ?? sleepMock.Object,
            Options.Create(new BarRetryWrapperService.ConfigurationModel
            {
    ...
    [Fact]
    public async Task RunAsync_PassesThrough_BarRecordNotFoundException()
    {
        var sut = CreateSut();

        var ex = await Assert.ThrowsAsync<BarRecordNotFoundException>(() =>
            sut.RunAsync<int>(_ => throw new BarRecordNotFoundException(5),
                TestContext.Current.CancellationToken));
    ...
    [Fact]
    public async Task RunAsync_WrapsExpectedNonTransient_AsBarConnectorException()
    {
        var sut = CreateSut();
